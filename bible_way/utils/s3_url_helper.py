...
def get_presigned_url(url_or_key: str, expiration: Optional[int] = None) -> str:
    """
    Convert S3 URL or key to presigned URL or public URL.
    ...
    Returns:
        Presigned URL if presigned URLs are enabled, otherwise returns public URL
    """
    if not url_or_key:
        return ""
    ...
    if not use_presigned:
        ...
        return get_public_url(url_or_key)
    ...
    try:
        ...
        # If generation fails, return public URL for backward compatibility
        return get_public_url(url_or_key)
...
def get_public_url(url_or_key: str) -> str:
    ...
    # Extract key if full URL provided
    key = _extract_s3_key_from_url(url_or_key)

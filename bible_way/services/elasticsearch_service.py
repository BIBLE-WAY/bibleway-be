...
class ElasticsearchService:
    ...
    def search_chapters(self, book_id: str, language_id: str, search_text: str, 
        ...
        """
        Search for text within chapter blocks with highlighting and intelligent ranking.
        
        Results are ranked by relevance:
        1. Exact phrase matches (highest priority)
        2. Exact word matches (all words present)
        3. Partial word matches (some words present)
        4. Fuzzy matches (handles typos, lowest priority)
        ...
        Returns:
            List of matching blocks with chapter context and highlighted text, sorted by relevance
        """
        if not self.client:
        ...
        try:
            ...
            normalized_search = ' '.join(search_text.strip().split())
            
            bool_query = Q('bool',
                should=[
                    Q('match_phrase', text={'query': normalized_search, 'boost': 10.0}),
                    Q('match', text={'query': normalized_search, 'boost': 5.0, 'operator': 'and'}),
                    Q('match', text={'query': normalized_search, 'boost': 2.0, 'operator': 'or'}),
                    Q('match', text={'query': normalized_search, 'fuzziness': 'AUTO', 'boost': 1.0})
                ],

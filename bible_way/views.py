...
from bible_way.interactors.create_chapter_feedback_interactor import CreateChapterFeedbackInteractor
...
from bible_way.interactors.get_post_by_id_interactor import GetPostByIdInteractor
...
from bible_way.interactors.admin.get_chapter_feedbacks_interactor import AdminGetChapterFeedbacksInteractor
...
from bible_way.interactors.like_chapter_interactor import LikeChapterInteractor
from bible_way.interactors.unlike_chapter_interactor import UnlikeChapterInteractor
...
from bible_way.interactors.admin.delete_promotion_interactor import DeletePromotionInteractor
...
from bible_way.interactors.bulk_delete_chapters_interactor import BulkDeleteChaptersInteractor
...
from bible_way.interactors.get_chapter_metadata_interactor import GetChapterMetadataInteractor
...
def update_post_view(request):
    ...
    title = request.data.get('title')
    description = request.data.get('description')
    media_urls = request.data.get('media_urls')  # List of S3 URLs
    ...
    response = UpdatePostInteractor(storage=UserDB(), response=UpdatePostResponse()).\
        update_post_interactor(post_id=post_id, user_id=user_id, title=title, description=description, media_urls=media_urls)
    return response
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
def get_post_by_id_view(request):
    current_user_id = str(request.user.user_id)
    post_id = request.data.get('post_id', '').strip()
    
    response = GetPostByIdInteractor(storage=UserDB(), response=GetPostByIdResponse()).\
        get_post_by_id_interactor(post_id=post_id, current_user_id=current_user_id)
    return response
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
def like_chapter_view(request):
    user_id = str(request.user.user_id)
    chapter_id = request.data.get('chapter_id')
    
    response = LikeChapterInteractor(storage=UserDB(), response=LikeChapterResponse()).\
        like_chapter_interactor(chapter_id=chapter_id, user_id=user_id)
    return response

@api_view(['POST'])
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
def unlike_chapter_view(request):
    user_id = str(request.user.user_id)
    chapter_id = request.data.get('chapter_id')
    
    response = UnlikeChapterInteractor(storage=UserDB(), response=UnlikeChapterResponse()).\
        unlike_chapter_interactor(chapter_id=chapter_id, user_id=user_id)
    return response
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated, IsAdminUser])
def admin_delete_promotion_view(request):
    promotion_id = request.data.get('promotion_id', '').strip()
    
    if not promotion_id:
        return DeletePromotionResponse().validation_error_response("promotion_id is required in request body")
    
    response = DeletePromotionInteractor(storage=UserDB(), response=DeletePromotionResponse()).\
        delete_promotion_interactor(promotion_id=promotion_id)
    return response
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
def get_chapter_metadata_view(request):
    chapter_id = request.data.get('chapter_id')
    user_id = str(request.user.user_id) if request.user.is_authenticated else None

    response = GetChapterMetadataInteractor(
        storage=UserDB(),
        response=GetChapterMetadataResponse()
    ).get_chapter_metadata_interactor(
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated, IsAdminUser])
def admin_get_chapter_feedbacks_view(request):
    response = AdminGetChapterFeedbacksInteractor(storage=UserDB(), response=AdminGetChapterFeedbacksResponse()).\
        get_chapter_feedbacks_interactor()
    return response
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
def create_chapter_feedback_view(request):
    user_id = str(request.user.user_id)
    chapter_id = request.data.get('chapter_id')
    description = request.data.get('description')
    rating = request.data.get('rating')
    
    response = CreateChapterFeedbackInteractor(storage=UserDB(), response=CreateChapterFeedbackResponse()).\
        create_chapter_feedback_interactor(chapter_id=chapter_id, user_id=user_id, description=description, rating=rating)
    return response
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated, IsAdminUser])
def bulk_delete_chapters_view(request):
    chapter_ids = request.data.get('chapter_ids', [])
    
    response = BulkDeleteChaptersInteractor(storage=UserDB(), response=BulkDeleteChaptersResponse()).\
        bulk_delete_chapters_interactor(chapter_ids=chapter_ids)
    return response
